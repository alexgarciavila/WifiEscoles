...
def mock_ctk_modules():
    ...
    with (
        patch("wifi_connector.gui.main_window.ctk.CTk") as mock_ctk,
        ...
        patch(
            "wifi_connector.gui.main_window.ctk.set_appearance_mode"
        ) as mock_appearance,
        patch(
            "wifi_connector.gui.main_window.ctk.set_default_color_theme"
        ) as mock_theme,
    ):
...
def mock_vault_unlock():
    def _fake_unlock(self):
        try:
            self.credentials_manager.load_credentials("test")
    ...
    with patch(
        "wifi_connector.gui.main_window.MainWindow._unlock_and_load_vault",
        _fake_unlock,
    ):
        yield
...
class TestMainWindowInit:
    ...
    def test_initializes_window_with_correct_properties(
        self, mock_creds_manager_class, mock_ctk_modules
    ):
        ...
        mock_ctk_modules["window"].title.assert_called_once_with(
            "Wifi de Centres Educatius de Catalunya"
        )
    ...
    def test_loads_credentials_on_init(
        self, mock_creds_manager_class, mock_ctk_modules, mock_credentials_manager
    ):
...
class TestUpdateStatus:
    ...
    def test_updates_status_with_success_color(
        ...
        main_window.status_label.configure.assert_called_once_with(
            text="Success message", text_color="#2ecc71"
        )
    ...
    def test_updates_status_with_error_color(
        ...
        main_window.status_label.configure.assert_called_once_with(
            text="Error message", text_color="#e74c3c"
        )
    ...
    def test_updates_status_with_info_color(
        ...
        main_window.status_label.configure.assert_called_once_with(
            text="Info message", text_color="#3498db"
        )
...
class TestSearchFiltering:
    ...
    def test_filter_centers_with_empty_query_shows_all(
        ...
        assert (
            main_window.all_centers
            == mock_credentials_manager.get_all_centers.return_value
        )
    ...
    def test_filter_centers_shows_no_results_message(
        ...
        main_window.center_count_label.configure.assert_called_with(
            text="No s'han trobat resultats", text_color="#e74c3c"
        )
...
class TestCenterSelection:
    ...
    def test_on_center_selected_sets_selected_center(
        ...
        main_window = MainWindow()
        setattr(main_window, "connect_button", MagicMock())
...
class TestConnectionActions:
    ...
    def test_on_connect_profile_clicked_disables_button_during_connection(
        ...
        main_window.selected_center = (
            mock_credentials_manager.get_all_centers.return_value[0]
        )

        main_window._on_connect_profile_clicked()

        main_window.connect_profile_button.configure.assert_called_with(
            state="disabled", text="Connectant..."
        )
